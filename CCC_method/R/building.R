building <- function(X1Y, ny, model) {
  ...
  if (model == "b_ppls") {
    pena <- ppls::ppls.splines.cv(as.matrix(X1Y[,10:18]), as.numeric(X1Y[,ny]), lambda = seq(-1000, 1000, 1), k=10, scale = T, reduce.knots= TRUE)
    lambda <- pena$lambda.opt
    ncomp <- pena$ncomp.opt
    dummy <- ppls::X2s(as.matrix(X1Y[,10.18]), reduce.knots = TRUE)
    P <- ppls::Penalty.matrix(m = ncol(dummy$Z))
    model <- ppls::penalized.pls.cv(as.matrix(dummy$Z), P = P, lambda = lambda, as.numeric(X1Y[,ny]), k=10)
  }
  return(model)
}